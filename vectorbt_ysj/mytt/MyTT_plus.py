    ...
    if isinstance(N, (int, float)):
        ...
        for i in range(len(S)):
            if (not np.isnan(N[i])) and N[i] <= i + 1:
                startIndex = i + 1 - N[i] if N[i] > 0 else 0  # 20250915 add by Weo. N为0时从第一个有效值开始算起(文华定义)
                res[i] = S[int(startIndex):i + 1].max()
        return res
...
def LLV(S, N):   #LLV,支持N为序列版本
    ...
    if isinstance(N, (int, float)):
        ...
        for i in range(len(S)):
            if (not np.isnan(N[i])) and N[i] <= i + 1:
                startIndex = i + 1 - N[i] if N[i] > 0 else 0  # 20250915 add by Weo. N为0时从第一个有效值开始算起(文华定义)
                res[i] = S[int(startIndex):i + 1].min()
        return res

def HV(S, N):  #HV,支持N为序列版本
    # type: (np.ndarray, Optional[int,float, np.ndarray]) -> np.ndarray
    """
    HV(C, 5)  # 昨天开始最近5天收盘最高价
    """
    return HHV(pd.Series(S).shift(1).values, N)

def LV(S, N):  #LV,支持N为序列版本
    # type: (np.ndarray, Optional[int,float, np.ndarray]) -> np.ndarray
    """
    LV(C, 5)  # 昨天开始最近5天收盘最低价
    """
    return LLV(pd.Series(S).shift(1).values, N)


def SUM(S, N):   #SUM,支持N为序列版本。按照文华公式的定义
    # type: (np.ndarray, Optional[int,float, np.ndarray]) -> np.ndarray
    """
    SUM(V, 5)  # 最近5天成交量总和
    """
    if isinstance(N, (int, float)):
        ...
        for i in range(len(S)):
            if (not np.isnan(N[i])) and N[i] <= i + 1:
                startIndex = i + 1 - N[i] if N[i] > 0 else 0  # 20250915 add by Weo. N为0时从第一个有效值开始算起(文华定义)
                res[i] = S[int(startIndex):i + 1].sum()
        return res
...
def DSMA(X, N):    # 偏差自适应移动平均线   type: (np.ndarray, int) -> np.ndarray
    ...
    RMS = np.sqrt(MyTT.SUM(np.square(Filt), N) / N)
    ScaledFilt = Filt / RMS
    alpha1 = np.abs(ScaledFilt) * 5 / N    
    return MyTT.DMA(X, alpha1)
