...
def execute(symbol: str, start_date: datetime, end_date: datetime, interval: Interval, length: int, stpr: int, n: int):
    ...
    if klines_close is not None and not klines_close.empty:
        ...
        dt_list = pd.Series([dt.timestamp() for dt in klines_close.index])
        dt_list.index = pd.to_datetime(klines_close.index)
        # calculate_signals(klines_close[symbol], klines_high[symbol], klines_low[symbol], klines_vol[symbol], dt_list,
        #                   interval, length, stpr, n)

        """"""
        indicator_factory = vbt.IndicatorFactory(
            input_names=['close_price', 'high_price', 'low_price', 'vols', 'dt_list'],
            param_names=['interval', 'length', 'stpr', 'n'],
            output_names=['l_open_sig', 'l_close_sig', 's_open_sig',
                          's_close_sig'])
        outs = indicator_factory.from_apply_func(calculate_signals).run(klines_close[symbol], klines_high[symbol],
                                                                        klines_low[symbol], klines_vol[symbol], dt_list,
                                                                        interval, length, stpr, n)
        print(f'>>l_open_sig={outs.l_open_sig}')
        print(f'>>l_close_sig={outs.l_close_sig}')
        print(f'>>s_open_sig={outs.s_open_sig}')
        print(f'>>s_close_sig={outs.s_close_sig}')


def calculate_signals(close_price: np.ndarray, high_price: np.ndarray, low_price: np.ndarray, vols: np.ndarray,
                      dt_list: np.ndarray, interval: Interval, length: int, stpr: int, n: int):
    """计算交易信号"""
    """#####################################################################
    注意，要对接verctorBT的IndicatorFactory方式，需要改造这几个函数，改造成支持多维数组，可参考官方自带的MA实现方式（具体ma计算在
    generic/nb.py的rolling_mean_nb()）
    #####################################################################"""
    if close_price is not None and len(close_price) > 0:
        dt_list = [datetime.fromtimestamp(i[0], tz=pytz.UTC) for i in dt_list]
        cnn: int = cal_kline_len_single_day(close_price, dt_list, interval)
        print(f'>>cnn={cnn}')
        zd: np.ndarray = cal_zd(close_price, n, cnn)
        # print(f'>>zd[-10:]={zd[-10:]}')
        zd_start_dt = dt_list[np.where((zd == 1) & (MyTT.REF(zd, 1) == 0))[0]]
        # zd_start_dt = [datetime.fromtimestamp(dt) for dt in zd_start_dt]
        zd_end_dt = dt_list[np.where((zd == 0) & (MyTT.REF(zd, 1) == 1))[0]]
        # zd_end_dt = [datetime.fromtimestamp(dt) for dt in zd_end_dt]
        print(f'>>转震荡信号点={zd_start_dt}')
        print(f'>>结束震荡信号点={zd_end_dt}')
        ...
        # bsl = np.array([bsr[i - 1] if i > 0 and datetime.fromtimestamp(dt_list[i - 1]).hour == last_bar_hour and
        #                               datetime.fromtimestamp(dt_list[i - 1]).minute == last_bar_minute else np.nan
        #                 for i in range(len(close_price))])
        bsl = np.array([bsr[i - 1] if i > 0 and dt_list[i - 1].hour == last_bar_hour and
                                      dt_list[i - 1].minute == last_bar_minute else np.nan
                        for i in range(len(close_price))])
        ...
        while not finish:
            ...
            # dt_pre_final = np.nan if np.isnan(pre_final_close_index) else datetime.fromtimestamp(
            #     dt_list[pre_final_close_index])
            ...
            #         final_close_index={final_close_index}({datetime.fromtimestamp(dt_list[final_close_index])})')
            print(f'>>pre_final_close_index={pre_final_close_index}({dt_pre_final}), \
        ...
        lo_dt_list = dt_list[np.where(dk_l == 1)[0]]
        # lo_dt_list = [datetime.fromtimestamp(dt) for dt in lo_dt_list]
        lc_dt_list = dt_list[np.where(dk_l == -1)[0]]
        # lc_dt_list = [datetime.fromtimestamp(dt) for dt in lc_dt_list]
        so_dt_list = dt_list[np.where(dk_l == 2)[0]]
        # so_dt_list = [datetime.fromtimestamp(dt) for dt in so_dt_list]
        sc_dt_list = dt_list[np.where(dk_l == -2)[0]]
        # sc_dt_list = [datetime.fromtimestamp(dt) for dt in sc_dt_list]
        print(f'>>多开信号={lo_dt_list}')
        print(f'>>多平信号={lc_dt_list}')
        print(f'>>空开信号={so_dt_list}')
        print(f'>>空平信号={sc_dt_list}')
...
def handle_close_operation(dk_l: np.ndarray, vols: np.ndarray, low_price: np.ndarray, high_price: np.ndarray,
                           zd: np.ndarray, stpr: int, stbar: int):
    ...
    jp_temp = MyTT.IF(signal == 1, MyTT_plus.HV(low_price, cn_l),
                      MyTT.IF(signal == 2, MyTT_plus.LV(high_price, cn_l), np.nan))
    ...
    sum_vol_l = MyTT.IF(cn_l > 0, MyTT_plus.SUM(vols, cn_l), np.nan)
    sum_amt_l = MyTT.IF(cn_l > 0, MyTT_plus.SUM(vols * vp_l, cn_l), np.nan)
    vwap_l = MyTT.IF(cn_l > 0, MyTT.IF(cn_l >= stbar, vp_l, sum_amt_l / sum_vol_l), np.nan)
    close_con = MyTT.IF(signal == 1, low_price <= vwap_l,
                        MyTT.IF(signal == 2, high_price >= vwap_l, np.nan))
...
def cal_kline_len_single_day(prices: np.ndarray, dt_list: np.ndarray, interval: Interval):
    ...
    if prices is not None and len(prices) > 0:
        ...
        start_minute = 0
        # 判断是否有夜盘K线
        night_bars_count = len([dt for dt in dt_list if dt.hour >= 21])
        day_minutes: list[int] = [dt.minute for dt in dt_list if dt.hour == 9]
        ...
        for i in range(len(prices) - 1, -1, -1):
            dt = dt_list[i]
            if (dt.hour == start_time_array[0] and dt.minute == start_time_array[1] and dt.second == 0):
                end_index = i
            elif (dt.hour == start_hour and dt.minute == start_minute and dt.second == 0 and end_index != -1):
                start_index = i
...
def cal_zd(prices: np.ndarray, n: int, cnn: int):
    """计算是否震荡"""
    zf: np.ndarray = MyTT.LN(prices / MyTT.REF(prices, 1)) * 100
    ...
    bf_left = MyTT.REF(ma_vlt >= MyTT_plus.HV(ma_vlt, ln1), ln1)
    bf_left2 = MyTT.REF(ma_vlt >= MyTT_plus.HV(ma_vlt, 20 * cnn), ln1)
    bg_left = MyTT.REF(ma_vlt <= MyTT_plus.LV(ma_vlt, ln1), ln1)
    bg_left2 = MyTT.REF(ma_vlt <= MyTT_plus.LV(ma_vlt, 20 * cnn), ln1)
    bf_left = [False if np.isnan(it) else it for it in bf_left]
    bf_left2 = [False if np.isnan(it) else it for it in bf_left2]
    bg_left = [False if np.isnan(it) else it for it in bg_left]
    bg_left2 = [False if np.isnan(it) else it for it in bg_left2]
