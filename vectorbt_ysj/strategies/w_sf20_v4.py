...
def calculate_signals(close_price: pd.Series, high_price: pd.Series, low_price: pd.Series, open_price: pd.Series,
    ...
    if close_price is not None and not close_price.empty:
        ...
        # 开仓信号
        bkcon = (close_price > ma240) & upband & (close_price > upavg) & (zd == 0)
        skcon = (close_price < ma240) & downband & (close_price < lowavg) & (zd == 0)
        ...
        handle_close_operation_2(dk_l, vols.values, low_price.values, high_price.values, zd, stpr, stbar)
    ...
    for i in range(len(dk_l)):
        if dk_sig != 0:  # 处理持仓状态下的K线
            if dk_l[i] != 0 and dk_l[i] != dk_sig:  # 反向，直接反手
                ...
                close_con = (low_price[i] <= vwap if dk_sig == 1 else high_price[i] >= vwap) or (
                        zd[i] == 1 and zd[i - 1] == 0)
